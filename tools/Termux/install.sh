...
setup_bz2sh() {
  ...
  test -x "$BZSH/tools/Termux/upgrade.sh" || test -f "$BZSH/tools/Termux/upgrade.sh" || {
    chmod -x "$BZSH/tools/Termux/upgrade.sh"  >/dev/null 2>&1 || {
      fmt_info "cannot chmod file upgrade.sh"
      echo "No such file upgrade.sh in directory $BZSH/tools/Termux"
      exit 1
    }
  }

  # Checking file 'uninstall.sh'
  test -x "$BZSH/tools/Termux/uninstall.sh" || test -f "$BZSH/tools/Termux/uninstall.sh" || {
    chmod -x "$BZSH/tools/Termux/uninstall.sh"  >/dev/null 2>&1 || {
      fmt_info "cannot chmod file uninstall.sh"
      echo "No such file uninstall.sh in directory $BZSH/tools/Termux"
      exit 1
    }
  }
